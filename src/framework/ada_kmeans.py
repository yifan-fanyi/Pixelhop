...
def Comupte_Cross_Entropy1(X, Y):
    ...
    if np.unique(Y).shape[0] == 1: #alread pure
        return 0
    clf = LogisticRegression(random_state=0, solver='lbfgs', multi_class='multinomial',class_weight='balanced').fit(X, Y.reshape(-1,))
    ...
    true_indicator = np.zeros((samp_num,cls_num))
    true_indicator[np.arange(samp_num),Y[:,0]] = 1
    #print(LL(true_indicator,prob)/math.log(cls_num))
    return LL(true_indicator,prob)/math.log(cls_num)
    ...
    for i in range(trial):
        ...
        if np.min(counting)>int(0.05*X['Data'].shape[0]):
            ...
            for k in range(sep_num):
                tH += weight[k]*Comupte_Cross_Entropy1(X['Data'][kmeans.labels_ == k], X['Label'][kmeans.labels_ == k])
            #tH /= float(sep_num)
            if tH < H:
                H = tH
