...
def myModel(x, getK=1):
    x1 = PixelHop_Unit(x, dilate=np.array([1]), pad='reflect', num_AC_kernels=9, weight_name='pixelhop1.pkl', getK=getK)
    x2 = PixelHop_Unit(x1, dilate=np.array([2]), pad='reflect', num_AC_kernels=25, weight_name='pixelhop2.pkl', getK=getK)
    x2 = AvgPooling(x2)
    x3 = PixelHop_Unit(x2, dilate=np.array([2]), pad='reflect', num_AC_kernels=35, weight_name='pixelhop3.pkl', getK=getK)
    x3 = AvgPooling(x3)
    x4 = PixelHop_Unit(x3, dilate=np.array([2]), pad='reflect', num_AC_kernels=55, weight_name='pixelhop4.pkl', getK=getK)
